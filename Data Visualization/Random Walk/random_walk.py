...
class RandomWalk:
    ...
    def fill_walk(self):
        ...
        while len(self.x_values) < self.num_points:
            x_direction = choice(
                [1, -1]
            )  # decides movement in the left, right, or vertical direction
            x_distance = choice(
                [0, 1, 2, 3, 4]
            )  # decides the distance to be walked in the chosen direction
            x_walk = x_distance * x_direction

            y_direction = choice(
                [1, -1]
            )  # decides upward, downward, or horizontal movement
