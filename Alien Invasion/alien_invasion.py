...
def run_game():
    ...
    while True:
        functions.check_events(
            game_settings, window, stats, score, play_button, spaceship, aliens, bullets
        )

        if stats.game_running:
            spaceship.update()
            functions.update_bullets(
                game_settings, window, stats, score, spaceship, aliens, bullets
            )
            functions.update_aliens(
                game_settings, stats, score, window, spaceship, aliens, bullets
            )

        functions.update_screen(
            game_settings, window, stats, score, spaceship, bullets, aliens, play_button
        )
